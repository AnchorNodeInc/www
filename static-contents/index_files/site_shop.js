			    ...
			    switch(hash_temp[0]){
				    ...
				    default:
					    ...
					    if(options.shop_view_tab_display === "N"){
						    if(is_mobile_width) SITE_SHOP_DETAIL.changeContentTab(target_tab, review_page, qna_page, false, false);
						    else SITE_SHOP_DETAIL.changeContentPCTab(target_tab, review_page, qna_page, false, false);
					    }
		    ...
		    switch (type) {
			    ...
			    case SHOP_CONST.TAB_TYPE.REVIEW:
				    $.ajax({
					    ...
					    data: { 'prod_idx': current_prod_idx, 'review_page': review_page, 'qna_page': qna_page, 'only_photo': only_photo, 'member_hash': MEMBER_HASH },
					    url: ('/shop/prod_review_mobile_html.cm'),
					    dataType: 'html',
					    async: false,
					    success: function (result) {
...
var SHOP_REVIEW_COMMENT = function () {
    ...
    var submit = function (t, type, i) {
        ...
        switch (type) {
            case 'main': // pc 리뷰 댓글
	            if (isMobile_width) {
		            var data = $('.categorize-mobile #' + $form.attr('id')).serializeObject();
	            } else {
		            var data = $('.categorize.review-box #' + $form.attr('id')).serializeObject();
	            }
                break;
            case 'sub_form': //pc 리뷰 대댓글
	            if (isMobile_width) {
					var data = $('.categorize-mobile #sub_review_form_' + i).serializeObject();
	            } else {
					var data = $('.categorize.review-box #sub_review_form_' + i).serializeObject();
	            }
                break;
            case 'editor': // pc 리뷰 댓글 수정
                if (isMobile_width) {
		            var data = $('.categorize-mobile #sub_review_editor_form_' + i).serializeObject();
	            } else {
		            var data = $('.categorize.review-box #sub_review_editor_form_' + i).serializeObject();
	            }
                break;
